...
install() {
    bash <(curl -Ls https://github.com/ashrarhacka409/PhoenixPars-x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    read -rp "This function will update the phoenix X-UI panel to the latest version. Data will not be lost. Whether to continues? [Y/N]: " yn
    if [[ $yn =~ "Y"|"y" ]]; then
        systemctl stop x-ui
        ...
        last_version=$(curl -Ls "https://github.com/ashrarhacka409/PhoenixPars-x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/') || last_version=$(curl -sm8 https://github.com/ashrarhacka409/PhoenixPars-x-ui/main/config/version)
        if [[ -z "$last_version" ]]; then
            red "Detecting the phoenix X-UI version failed, please make sure your server can connect to the GitHub API"
            exit 1
        fi
        
        yellow "The latest version of X-UI is: $ {last_version}, starting update..."
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/ashrarhacka409/PhoenixPars-x-ui/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
            red "Download the phoenix X-UI failure, please make sure your server can connect and download the files from github"
            exit 1
        fi
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/ashrarhacka409/PhoenixPars-x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_usage() {
    green "PhoenixPars X-UI English v${last_version} Installation is Completed, The Panel has been Started"
    echo -e ""
    echo -e "${GREEN} --------------------------------------------------------------------- ${PLAIN}"
...
show_menu() { phoen
    echo -e "
 -------------------------------------------------------------------------------- 
  ${GREEN}  ________ __                                   _              ${PLAIN} 
  ${GREEN}  |  __  | | |      ________ _______   _______ |_|  __     __     ${PLAIN}
  ${GREEN}  | |__| | | |____  |  __  | |  ___ | |  ___ |  _   \ \   / /      ${PLAIN}
  ${GREEN}  |  ____| |  ___ | | |  | | | |__| | | |  | | | |   \ \_/ /       ${PLAIN}
  ${GREEN}  | |      | |  | | | |  | | | |____| | |  | | | |   |  _  |    ${PLAIN} 
  ${GREEN}  | |      | |  | | | |__| | | |____  | |  | | | |   / / \ \    ${PLAIN}
  ${GREEN}  |_|      |_|  |_| |______| |______| |_|  |_| |_|  /_/   \_\    ${PLAIN}
  ${GREEN}                                                               ${PLAIN}
--------------------------------------------------------------------------------
  ${GREEN}PHOENIX PARS X-UI PANEL MANAGEMENT SCRIPT ${PLAIN}
--------------------------------------------------------------------------------
